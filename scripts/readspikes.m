...
%READSPIKES is used to read spikes generated by UCI-SNN
%Output is matrix S = (time x nID), indices start at 1.
%   FILE:       path to spike file (expects data to be in Adress-Event
%               Representation AER: spike time (ms) followed by neuron ID (both
%               uint32))
%   FRAMEDUR:   binning size (ms). S will contain # spikes a neuron emitted per
%               FRAMEDUR. If -1, S will be returned in AER format [times;nIDs]
%
%   Ver 05/31/12
