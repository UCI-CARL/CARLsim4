...
/* Kris Carlson 10/31/2011
   This file defines the do functions called in pti_make_algo_scalar_es.cpp.
   These functions are necessary because they return eoModlibAlgo objects and
   not eoAlgo objects.  Also, the new function calls the correct pti_eoEasyEA
   constructor.  

   Programming Note: Considered placing Parser commands for self-adaptive
   mutation operators in this file but left them in the driver file.  This
   is because there is no need to define these values if eoReal is used.
*/
//-----------------------------------------------------------------------------
...
//I also had to include the pti_eoEasyEA.h file
...
template <class EOT>
eoModlibAlgo<EOT> & do_pti_make_algo_scalar(eoParser& _parser, eoState& _state, eoEvalFunc<EOT>& _eval, eoContinue<EOT>& _continue, eoGenOp<EOT>& _op, eoDistance<EOT> * _dist = NULL)
{
  ...
  // now the pti_eoEasyEA, which is the only thing changed in this
  //file
  /*
    //the original line
  ...
  */
  //the new line that uses the eoModlib classes to appropriately call the correct functions
  //Define these parameter values for the parameter file                        
  _parser.createParam(true, "Isotropic", "Isotropic self-adaptive mutation", 'i\
', "ES mutation");                                                              
  _parser.createParam(false, "Stdev", "One self-adaptive stDev per variable", '\
s', "ES mutation");                                                             
  _parser.createParam(false, "Correl", "Use correlated mutations", 'c', "ES mut\
ation");   
  eoModlibAlgo<EOT> *algo = new pti_eoEasyEA<EOT>(_continue, _eval, *breed, *replace);
  _state.storeFunctor(algo);
